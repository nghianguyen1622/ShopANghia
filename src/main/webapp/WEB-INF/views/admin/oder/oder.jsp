			...
			<div id="sub-main">
				...
				<div class="row space-top">
					<div class="col-xs-12 col-sm-5 col-md-7 col-lg-7">
						<div class="btn btn-danger btn-lg">
							<a href="themsanpham.html">Thêm mới</a>
						</div>
					</div>
				...
				<div class="row space-top box-total">
					<div class="col-xs-12 col-sm-8 col-md-7 col-lg-6">
						<span><i>Tổng số đơn hàng: </i><strong>${ oderbills.size() }</strong>
							|</span> <span><i>Có </i><strong>${ quantyconfirm.size() }</strong> <i>Đơn hàng chờ xác nhận</i></span>
					</div>
				</div>
				...
				<div class="row margin0">
					<div class="table-responsive table-sanpham">
						<table class="table table-striped table-bordered">
							...
							<c:forEach var="item" items="${ oderbills }">
								<tr>
									<td><input type="checkbox" value="" /></td>
									<td><a href="<c:url value="/orderdetail/${item.id}"/>">${ item.id }</a></td>
									<td>${ item.name }</td>
									<td>${ item.phone }</td>
									<td>${ item.quanty }</td>
									<td>${ item.total }</td>
									...
									<td>
										...
										<button><a href="<c:url value="/confirm/${item.id}"/>">Xác nhận</a></button>
										<button><a href="<c:url value="/delivery/${item.id}"/>">Đang giao</a></button>
										<button><a href="<c:url value="/success/${item.id}"/>">Hoàn thành</a></button>
